			...
			<div class="card "> 
				<div class="card-header pt-5">
					...
					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
						@if(in_array("cvideo_all",$rolerawdata, TRUE) || in_array("cvideo_create",$rolerawdata, TRUE)|| in_array("kt_roles_select_all",$rolerawdata, TRUE))
							<a href="{{ route('add.cvideo') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
				...
				<div class="card-body pt-0"> 
					<table class="table border align-middle rounded  dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
						...
						<tbody class="fw-semibold text-gray-600">
							<?php $j=0; ?>    
							@foreach($cvideo as $tvideos)
							<tr>
								<td></td>
								<td>{{$j+=1;}}</td> 
								<td >{{ $tvideos->c_name }}</td>
								<!-- <td>
                                        @if($tvideos->c_url)
								...
								<td >{{ $tvideos->p_video }}</td>
								<td >{{ $tvideos->c_description }}</td>
							...
							<!-- <td><label class="form-check form-switch form-switch-sm form-check-custom form-check-solid flex-stack mb-0">    
								...
								<td>
									@if(in_array("cvideo_all",$rolerawdata, TRUE) || in_array("cvideo_read",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<a href="{{route('view.cvideo', $tvideos->id)}}" class="btn btn-sm btn-warning align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 10px;">
										<i class="fa fa-eye" aria-hidden="true"></i>
									</a>    	
									@endif
									@if(in_array("cvideo_all",$rolerawdata, TRUE) || in_array("cvideo_write",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))             
									<a href="{{route('edit.cvideo', $tvideos->id)}}" class="btn btn-sm btn-info align-self-center" style="border-radius: 104px;padding: 8px 8px 10px 13px;margin: 0px 1px;">
										<i class="fa fa-edit" aria-hidden="true"></i>
									</a>
									@endif
									@if(in_array("cvideo_all",$rolerawdata, TRUE) || in_array("cvideo_delete",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
