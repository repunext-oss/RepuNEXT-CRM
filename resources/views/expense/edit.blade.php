			...
			<div class="card "> 
				<div class="card-header pt-5">
					<h3 class="card-title align-items-start flex-column">
						<span class="card-label fw-bold fs-3 mb-1">Edit Expense</span>
						<span class="text-muted fw-semibold fs-7">
							<a href="{{route('revenue-expense.index')}}" class="text-muted text-hover-primary">Revenue & Expense</a> /
							<a href="{{route('expense.show', $expense)}}" class="text-muted text-hover-primary">View</a> /
				...
				<div class="card-body border-0 pt-0"> 
					<form action="{{ route('expense.update', $expense) }}" method="post" class="form" enctype="multipart/form-data">
						@csrf
						@method('PUT')
							...
							<div class="row g-4">
								<div class="col-lg-4">
									...
									<input
										...
										value="{{ old('e_name', $expense->e_name) }}"
										maxlength="191">
								</div>
						...
						<div class="d-flex justify-content-end mt-6 gap-3">
							<a href="{{ route('expense.show', $expense) }}" class="btn btn-light btn-lg px-6">
								Cancel
							</a>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Payment method options for expense
  ...
  const currentCategory = @json($expense->category);
  const currentSubcategory = @json($expense->subcategory);
  const currentPaymentMethod = @json($expense->payment_method);
  ...
  function populateCategories(selectedCategory = null) {
    ...
    Object.keys(CATEGORIES_DATA.expense).forEach(cat => {
  ...
  function populateSubcategories(category, selectedSubcategory = null) {
    const subcategories = CATEGORIES_DATA.expense[category] || [];
  ...
  /**
   * Populate payment methods for expense
  ...
  function populatePaymentMethods(selectedPaymentMethod = null) {
    ...
    const methods = PAYMENT_METHODS.expense;
