			...
			<div class="card "> 
				<div class="card-header pt-5">
					...
					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
							@if(in_array("goalsheet_category_all",$rolerawdata, TRUE) || in_array("goalsheet_category_create",$rolerawdata, TRUE)|| in_array("kt_roles_select_all",$rolerawdata, TRUE))
							<a href="{{ route('add.gscategory') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
				...
				<div class="card-header align-items-center py-5 gap-2 gap-md-5 border-0"> 
					...
					<div class="card-toolbar flex-row-fluid justify-content-end gap-5"> 
						...
						@if(in_array("goalsheet_category_all",$rolerawdata, TRUE) || in_array("goalsheet_category_create",$rolerawdata, TRUE) || in_array("goalsheet_category_write",$rolerawdata, TRUE) || in_array("kt_roles_select_all",$rolerawdata, TRUE))
				...
				<div class="card-body pt-0"> 
					<table class="table border align-middle rounded  dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase text-muted bg-light">
							...
							<th class="min-w-125px sorting">Goal Sheet Category</th>
						...
						<tbody class="fw-semibold text-gray-600">
							...
							<tr>
								...
								<td>
								@if(in_array("goalsheet_category_all",$rolerawdata, TRUE) || in_array("goalsheet_category_create",$rolerawdata, TRUE) || in_array("goalsheet_category_write",$rolerawdata, TRUE)  || in_array("goalsheet_category_delete",$rolerawdata, TRUE) || in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<label class="form-check form-switch form-switch-sm form-check-custom form-check-solid flex-stack mb-0">    
								<input class="form-check-input" type="checkbox" onchange="Check(this,{{$repns->id}})" @if($repns->state_status==0) checked @endif></label>
								@endif      
								<td>
									@if(in_array("goalsheet_category_all",$rolerawdata, TRUE) || in_array("goalsheet_category_read",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<a href="{{route('view.gscategory', $repns->id)}}" class="btn btn-sm btn-warning align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 10px;">
										<i class="fa fa-eye" aria-hidden="true"></i>
									</a> 
									@endif
									@if(in_array("goalsheet_category_all",$rolerawdata, TRUE) || in_array("goalsheet_category_write",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))         
									<a href="{{route('edit.gscategory', $repns->id)}}" class="btn btn-sm btn-info align-self-center" style="border-radius: 104px;padding: 8px 8px 10px 13px;margin: 0px 1px;">
										<i class="fa fa-edit" aria-hidden="true"></i>
									</a>
									@endif
									@if(in_array("goasheet_category_all",$rolerawdata, TRUE) || in_array("goalsheet_category_delete",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))         
...
<script>  
	function deleteConfirmation(id)
 	{
		swal({
			...
			let _url = '/goalsheet/category/destroy/${id}`;
			$.ajax({
				type: 'POST',  
				url: _url,
				data: {_token: token},  
	...
	function Check(value,id) {  
		...
		let token = "{{ csrf_token() }}";
			let _url = `/goalsheet/category/status`; 
			$.ajax({
				type: 'POST',  
				url: _url,
