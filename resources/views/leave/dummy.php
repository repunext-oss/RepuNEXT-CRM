											...
											<tr> 
												...
												<td>
													@if(in_array("leave_all", $rolerawdata, TRUE) || in_array("leave_read", $rolerawdata, TRUE) || in_array("kt_roles_select_all", $rolerawdata, TRUE))
														<a href="{{ route('view.leave', $repns->id) }}" class="btn btn-sm text-white" style="background-color: #002244; text-transform: none;">
										...
										@if(in_array("kt_roles_select_all", $rolerawdata, TRUE) || in_array("leave_all", $rolerawdata, TRUE) || (Auth::user()->id) == ($repns->name))
											<tr class="leave-row" data-status="{{ $repns->l_status }}" id="row-{{ $repns->id }}">
												...
												<td>
													@if(in_array("leave_all",$rolerawdata, TRUE) || in_array("kt_roles_select_all",$rolerawdata, TRUE))
													<!-- Approve Button -->
													<button class="approve-btn btn btn-success btn-sm" data-id="{{ $repns->name }}" data-leave-id="{{ $repns->id }}" id="approve-btn-{{ $repns->id }}" data-totaldays="{{ $repns->totaldays }}">
														Approve
													</button>
													@endif
													@if(in_array("leave_all",$rolerawdata, TRUE) || in_array("kt_roles_select_all",$rolerawdata, TRUE))
													<!-- Rejection Reason Modal -->
													<button class="reject-btn btn btn-danger btn-sm" data-id="{{ $repns->name }}" data-leave-id="{{ $repns->id }}">
														Reject
													</button>
													@endif
													@if(in_array("leave_all",$rolerawdata, TRUE)||in_array("leave_read",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
							...
							<table class="table border align-middle rounded dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
								...
								<?php $j = 0; ?>
									@foreach($leaveSummary as $summary)
										<tr>
											<td></td>
											<td>{{ ++$j }}</td>											<td>{{ $summary->user_name }}</td> <!-- Display User Name -->
											<td>{{ $summary->year }}</td>
											<td>{{ date('F', mktime(0, 0, 0, $summary->month, 1)) }}</td> <!-- Displaying Month Name -->
											<!-- <td>{{ ($summary->total_leave)/9}}</td> -->
											<td>{{ ($summary->total_leave)/8 }}</td>
										</tr>
									@endforeach
								</tbody>
							</table>
...
$(".approve-btn, .reject-btn").click(function () {
    ...
    var newLStatus = action === "approve" ? 1 : 2;
    ...
    // If approve, continue directly
    updateLeaveStatus(leaveId, userName, action, newLStatus, "");
});
...
$("#submit-reject-btn").click(function () {
    ...
    const userName = $(`.reject-btn[data-leave-id="${leaveId}"]`).data("id");
    ...
    updateLeaveStatus(leaveId, userName, "reject", 2, reason);
    $("#rejectModal").hide();
    $("#reject-reason").val("");
});
    ...
    <div class="modal-dialog">
        <div class="modal-content p-4">
            ...
            <div class="modal-body">
                ...
                <input type="hidden" id="reject-user-name">
...
$(document).ready(function () {
    ...
    $("#submit-reject-btn").click(function () {
        const leaveId = $("#reject-leave-id").val();
        const userName = $("#reject-user-name").val();
        const reason = $("#reject-reason").val().trim();
