			...
			<div class="card "style="background-color: #E1EBEE"> 
				<div class="card-header pt-5">
					...
					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
							...
							<a href="{{ route('add.leaveManagement') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
								<span class="svg-icon svg-icon-2">
									<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
										<rect opacity="0.5" x="11.364" y="20.364" width="16" height="2" rx="1" transform="rotate(-90 11.364 20.364)" fill="currentColor" />
										<rect x="4.36396" y="11.364" width="16" height="2" rx="1" fill="currentColor" />
									</svg>
								</span>Add Credit Leave</button>
							</a>
							@endif
									...
									<tbody class="fw-semibold text-gray-600">
										...
										@foreach($repn as $repns)
											...
											<tr> 
												...
												<td>
													{{ $repns->l_status == 0 ? 'Pending' : ($repns->l_status == 1 ? 'Approved' : 'N/A') }}
												</td>												
												<td>
													<!-- @if(in_array("leave_all",$rolerawdata, TRUE) || in_array("kt_roles_select_all",$rolerawdata, TRUE)) -->
													<!-- Approve Button -->
...
@if(isset($leaveBalance))
    ...
    <p>Permission: {{ $leaveBalance->permission == 1 ? 'Granted' : 'Not Granted' }}</p>
    <p>Total Leave Balance: {{ $leaveBalance->sick_leave + $leaveBalance->casual_leave }}</p>
    <p>Credit Leave: {{ $leaveBalance->sick_leave + $leaveBalance->casual_leave - $leaveBalance->taken_leave }}</p>
@else
...
// Reusable function for update/approve
function updateLeaveStatus(leaveId, userName, action, l_status, reason) {
    $.ajax({
        ...
        data: {
            ...
            action: action,
            l_status: l_status,
            reason: reason // send only for reject
        },
        success: function (response) {
            ...
            $(`button.${action}-btn[data-leave-id="${leaveId}"]`)
                .text(statusText)
                .prop("disabled", true)
                .removeClass("approve-btn reject-btn")
            ...
            $("#row-" + leaveId).data("status", l_status);
        },
        error: function (xhr) {
...
$(document).ready(function() {
    $(".approve-btn").on("click", function() {
        var leaveId = $(this).data("leave-id"); // Get leave ID
        var username = $(this).data("username"); // Get username
        var totaldays = $(this).data("totaldays"); // Get totaldays value
        var userId = $(this).data("user-id"); // Get user ID

        if (!totaldays || totaldays == 0) {
            alert("Invalid leave days. Cannot approve.");
            return;
        }
        ...
        $.ajax({
            ...
            data: {
                leave_id: leaveId,
                username: username,
                totaldays: totaldays, // Send totaldays to backend
                _token: "{{ csrf_token() }}"
            },
            success: function(response) {
                // Create detailed message with deduction information
                var message = response.message + "\n\n";
                
                if (response.deduction_message) {
                    message += "Deduction Details:\n";
                    message += response.deduction_message + "\n\n";
                }
                
                if (response.used_hours) {
                    message += "Hours Used:\n";
                    if (response.used_hours.credit > 0) {
                        message += "• Credit Leave: " + response.used_hours.credit + " hours\n";
                    }
                    if (response.used_hours.cl > 0) {
                        message += "• Casual Leave: " + response.used_hours.cl + " hours\n";
                    }
                    if (response.used_hours.sl > 0) {
                        message += "• Sick Leave: " + response.used_hours.sl + " hours\n";
                    }
                    message += "• Total: " + response.used_hours.total + " hours\n\n";
                }
                
                if (response.new_balances_days) {
                    message += "Remaining Balance (Days):\n";
                    message += "• Credit Leave: " + response.new_balances_days.credit + " days\n";
                    message += "• Casual Leave: " + response.new_balances_days.cl + " days\n";
                    message += "• Sick Leave: " + response.new_balances_days.sl + " days\n";
                    if (response.new_balances_days.lop > 0) {
                        message += "• LOP: " + response.new_balances_days.lop + " days\n";
                    }
                }
                ...
                // Update UI
                $("#l_status-" + leaveId).text("Approved").css({
                    "color": "green",
                    "font-weight": "bold"
                });

                $(".approve-btn[data-leave-id='" + leaveId + "']")
                    .text("Approved")
                    .prop("disabled", true)
			...
			error: function(xhr) {
				...
				if (xhr.responseJSON && xhr.responseJSON.message) {
					...
					errorMsg = xhr.responseText; // fallback to raw response
				}
...
// Function to check current month leave balance
function checkCurrentMonthBalance(userId, callback) {
    $.ajax({
        url: "{{ url('/fetch-current-month-leave-balance') }}",
        method: "POST",
        data: {
            user_id: userId,
            _token: "{{ csrf_token() }}"
...
$(document).ready(function () {
    $('.approve-btn').on('click', function () {
        ...
        // Disable approve and reject buttons for this leaveId
        $('#approve-btn-' + leaveId).prop('disabled', true).text('Approved');
        $('#reject-btn-' + leaveId).prop('disabled', true);
    });
    ...
    $('.reject-btn').on('click', function () {
        ...
        // Disable approve and reject buttons for this leaveId
        $('#reject-btn-' + leaveId).prop('disabled', true).text('Rejected');
        $('#approve-btn-' + leaveId).prop('disabled', true);
    });
});
    ...
    <div class="modal-dialog">
        <div class="modal-content p-4">
            ...
            <div class="modal-body">
                ...
                <textarea id="reject-reason" class="form-control" placeholder="Enter rejection reason..."></textarea>
            </div>
            <div class="modal-footer">
                <button id="submit-reject-btn" class="btn btn-danger">Submit</button>
...
$(document).ready(function () {
    ...
    $(".reject-btn").click(function () {
        ...
        $("#reject-leave-id").val(leaveId);
        $("#reject-user-id").val(userId);
        $("#rejectModal").modal("show");
    });

    // Handle reject submit
    $("#submit-reject-btn").click(function () {
        const leaveId = $("#reject-leave-id").val();
        ...
        $.ajax({
            url: "{{ route('leaves.reject') }}",  // route defined in web.php
            method: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                leave_id: leaveId,
            ...
            success: function (response) {
                alert(response.message);  // This is the success message from the backend

                // Update the UI
                $(`.reject-btn[data-leave-id='${leaveId}']`)
                    .text("Rejected")
                    .prop("disabled", true)
                ...
                $(`.approve-btn[data-leave-id='${leaveId}']`)
                    .prop("disabled", true)
                    .css({ "background-color": "#ccc", "color": "#999" });

                $("#rejectModal").modal("hide");
                $("#reject-reason").val("");  // Clear the textarea
            },
            error: function (xhr) {
