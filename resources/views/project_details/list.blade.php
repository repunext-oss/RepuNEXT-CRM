			...
			<div class="card "> 
				<div class="card-header pt-5">
					<h3 class="card-title align-items-start flex-column">
						...
						<span class="text-muted fw-semibold fs-7"><a href="{{route('dashboard')}}" class="text-muted text-hover-primary">Home</a> / Project / Details</span>
					</h3>	
					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
						@if(in_array("project_all",$rolerawdata, TRUE) || in_array("project_create",$rolerawdata, TRUE)|| in_array("kt_roles_select_all",$rolerawdata, TRUE))
							<a href="{{ route('add.pdetail') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
				...
				<div class="card-body pt-0"> 
					<table class="table border align-middle rounded  dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase text-muted bg-light">
							...
							<th >#</th>
							<th class="min-w-115px sorting">Project Title</th> 
							<th class="min-w-115px sorting">Project Description</th> 
							<th class="min-w-115px sorting">Project Start Date</th>
							<th class="min-w-115px sorting">Project End Date</th>
							<th class="min-w-115px sorting">Service Category</th>
							<th class="min-w-115px sorting">project Timeline</th>
							<th class="min-w-115px sorting">Assigned To Member</th>
							<th class="min-w-115px sorting">Priority</th>
						...
						<tbody class="fw-semibold text-gray-600">
							...
							<tr>
								...
								<td >{{ $repns->project_title }}</td> 
								<td >{{ $repns->project_description }}</td> 
								<td >{{ $repns->project_start_date }} </td>
								<td>{{ $repns->project_end_date }} </td> 
								<td >
								<?php $member = (explode(",",$repns->project_service_category));?>
								...
								<td >{{ $repns->project_timeline }}</td> 

								<td >
								<?php $arraymember = (explode(",",$repns->assigned_to_member));?>

								@foreach($user as $users)
										@if(in_array ($users->id , $arraymember))
										
										{{$users->name."," }}
										@endif
								@endforeach
								</td>
								<td >{{ $repns->project_priority }}</td> 
								...
								<td>
									@if(in_array("project_all",$rolerawdata, TRUE) || in_array("project_read",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<a href="{{route('view.pdetail', $repns->id)}}" class="btn btn-sm btn-warning align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 10px;">
										<i class="fa fa-eye" aria-hidden="true"></i>
									</a>   
									@endif
									@if(in_array("project_all",$rolerawdata, TRUE) || in_array("project_write",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))             
									<a href="{{route('edit.pdetail', $repns->id)}}" class="btn btn-sm btn-info align-self-center" style="border-radius: 104px;padding: 8px 8px 10px 13px;margin: 0px 1px;">
										<i class="fa fa-edit" aria-hidden="true"></i>
									</a>
									@endif
									@if(in_array("project_all",$rolerawdata, TRUE) || in_array("project_delete",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
...
<script>  
	function deleteConfirmation(id)
 	{
		swal({
			...
			let _url = `/project/detail/destroy/${id}`;
			$.ajax({
				type: 'POST',  
				url: _url,
				data: {_token: token},  
	...
	function Check(value,id) {  
		...
		let token = "{{ csrf_token() }}";
			let _url = `/project/detail/status`; 
			$.ajax({
				type: 'POST',  
				url: _url,
