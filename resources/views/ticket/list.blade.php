			...
			<div class="card "> 
				<div class="card-header pt-5">
					...
					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
							<a href="{{ route('add.ticket') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
				...
				<div class="card-body pt-0"> 
					<table class="table border align-middle rounded  dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase text-muted bg-light">
							...
							<th >#</th>
							<th class="min-w-125px sorting">Ticket Id</th> 
							<th class="min-w-125px sorting">Subject </th> 
							<th class="min-w-125px sorting">Description</th>   
							<th class="min-w-100px sorting">Product</th> 
							<th class="min-w-100px sorting">Priority</th> 
							<th class="min-w-100px sorting">Status</th> 
							<th class="min-w-100px sorting">Action</th> 
						</tr> 
						</thead> 
						<tbody class="fw-semibold text-gray-600">
							...
							<tr>
								...
								<td>
                                        @php
                                            $member = explode(",", $repns->client_user_id);
                                        @endphp
								...
								<td >{{ $repns->ticket_subject }}</td> 
								<td >{{ $repns->ticket_description }}</td> 
								<td >
									<?php $gcategory = (explode(",",$repns->product));?>
									@foreach($gc as $gcs)
								...
								<td >{{ $repns->ticket_prority }}</td> 
								<td >{{ $repns->ticket_status}}</td> 
								...
								<td>
									...
									<a href="{{route('view.ticket', $repns->id)}}" class="btn btn-sm btn-warning align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 10px;">
									...
									<a href="{{route('edit.ticket', $repns->id)}}" class="btn btn-sm btn-info align-self-center" style="border-radius: 104px;padding: 8px 8px 10px 13px;margin: 0px 1px;">
