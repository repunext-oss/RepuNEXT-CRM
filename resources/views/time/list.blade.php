			...
			<div class="card "> 
				<div class="card-header pt-5">
					...
					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
						@if(in_array("task_timesheet_all",$rolerawdata, TRUE) || in_array("task_timesheet_create",$rolerawdata, TRUE)|| in_array("kt_roles_select_all",$rolerawdata, TRUE))
							<a href="{{ route('add.time') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
				...
				<div class="card-body pt-0"> 
					<table class="table border align-middle rounded  dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase text-muted bg-light">
							...
							<th >#</th>
							<th class="min-w-115px sorting">Task Name</th>
                            <th class="min-w-125px sorting">Starttime</th>
                            <th class="min-w-125px sorting">EndTime</th>
						...
						<tbody class="fw-semibold text-gray-600">
							...
							<tr>
                                ...
                                <td>
                                    @php
                                        $member = explode(",", $repns->goalid_ref);
                                    @endphp
                                ...
                                <td>{{ $repns->starttime }} </td>
								<td>{{ $repns->endtime }}</td>

						
								<!-- <input class="form-check-input" type="checkbox" onchange="Check(this,{{$repns->id}})" @if($repns->state_status==0) checked @endif></label></td>        -->
								<td>
									@if(in_array("task_timesheet_all",$rolerawdata, TRUE) || in_array("task_timesheet_read  ",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<a href="{{route('view.time', $repns->id)}}" class="btn btn-sm btn-warning align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 10px;">
										<i class="fa fa-eye" aria-hidden="true"></i>
									</a> 
									@endif         
									@if(in_array("task_timesheet_all",$rolerawdata, TRUE) || in_array("task_timesheet_write",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))      
									<a href="{{route('edit.time', $repns->id)}}" class="btn btn-sm btn-info align-self-center" style="border-radius: 104px;padding: 8px 8px 10px 13px;margin: 0px 1px;">
										<i class="fa fa-edit" aria-hidden="true"></i>
									</a>
									@endif
									@if(in_array("task_timesheet_all",$rolerawdata, TRUE) || in_array("task_timesheet_delete",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
