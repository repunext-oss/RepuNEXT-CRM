			...
			<div class="card "> 
				<div class="card-header pt-5">
					...
					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
							@if(in_array("time_details_all",$rolerawdata, TRUE) || in_array("time_details_create",$rolerawdata, TRUE)|| in_array("kt_roles_select_all",$rolerawdata, TRUE))
							<a href="{{ route('add.ptime') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
				...
				<div class="card-body pt-0"> 
					<table class="table border align-middle rounded  dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase text-muted bg-light">
							...
							<th >#</th>
							<th class="min-w-115px sorting">Project Ref Id</th> 
							<th class="min-w-115px sorting">Task_name</th> 
							<th class="min-w-115px sorting">Task Date</th>
							<th class="min-w-115px sorting">Task Assigned</th>
							<th class="min-w-115px sorting">Task Start Time</th>
							<th class="min-w-115px sorting">Task End Time</th>
							<th class="min-w-115px sorting">Task Description</th>
							<th class="min-w-115px sorting">Task Status</th>
						...
						<tbody class="fw-semibold text-gray-600">
							...
							<tr>
								...
								<td >
									 ...
									 @if($titles->id == $repns->project_ref_id)
								...
								<td >{{ $repns->task_name }} </td>
								<td>{{ $repns->task_date }} </td> 
								<td >{{ $repns->task_assigned }}</td> 
								<td >{{ $repns->task_start_time }}</td> 
								<td >{{ $repns->task_end_time }}</td> 
								<td >{{ $repns->task_description }}</td>
								...
								<td>
									@if(in_array("time_details_all",$rolerawdata, TRUE) || in_array("time_details_read  ",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<a href="{{route('view.ptime', $repns->id)}}" class="btn btn-sm btn-warning align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 10px;">
										<i class="fa fa-eye" aria-hidden="true"></i>
									</a>
									@endif        
									@if(in_array("time_details_all",$rolerawdata, TRUE) || in_array("time_details_write",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))      
									<a href="{{route('edit.ptime', $repns->id)}}" class="btn btn-sm btn-info align-self-center" style="border-radius: 104px;padding: 8px 8px 10px 13px;margin: 0px 1px;">
										<i class="fa fa-edit" aria-hidden="true"></i>
									</a>
									@endif
									@if(in_array("time_details_all",$rolerawdata, TRUE) || in_array("time_details_delete",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
	...
	function deleteConfirmation(id)
 	{
		swal({
			...
			let _url = `/project/timeline/destroy/${id}`;
			$.ajax({
				type: 'POST',  
				url: _url,
				data: {_token: token},  
