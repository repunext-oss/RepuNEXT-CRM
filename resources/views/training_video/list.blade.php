			...
			<div class="card "> 
				<div class="card-header pt-5">
					<h3 class="card-title align-items-start flex-column">
						...
						<span class="text-muted fw-semibold fs-7"><a href="{{route('dashboard')}}" class="text-muted text-hover-primary">Home</a> / Training / Video</span>
					</h3>

					<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
						@if(in_array("tvideo_all",$rolerawdata, TRUE) || in_array("tvideo_create",$rolerawdata, TRUE)|| in_array("kt_roles_select_all",$rolerawdata, TRUE))
							<a href="{{ route('add.tvideos') }}"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_user" >
				...
				<div class="card-body pt-0"> 
					<table class="table border align-middle rounded  dataTable table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_views_table">
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase text-muted bg-light">
							...
							<th class="min-w-75px sorting">Department Video</th> 
						...
						<tbody class="fw-semibold text-gray-600">
							<?php $j=0; ?>    
							@foreach($tvideo as $tvideos)
							<tr>
								<td></td>
								<td>{{$j+=1;}}</td> 
								<td >{{ $tvideos->tv_name }}</td>
								<!-- <td>
                                        @if($tvideos->c_url)
                                            <video width="200" controls>
                                                <source src="{{ asset('storage/' . $tvideos->c_url) }}" type="video/mp4">
								...
								<td >{{ $tvideos->tv_department }}</td>
								<td >{{ $tvideos->tv_description }}</td>
								
							<!-- <td><label class="form-check form-switch form-switch-sm form-check-custom form-check-solid flex-stack mb-0">    
								<input class="form-check-input" type="checkbox" onchange="Check(this,{{$tvideos->id}})" @if($tvideos->state_status==0) checked @endif></label></td>        -->
								<td>
									@if(in_array("tvideo_all",$rolerawdata, TRUE) || in_array("tvideo_read",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<a href="{{route('view.tvideos', $tvideos->id)}}" class="btn btn-sm btn-warning align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 10px;">
										<i class="fa fa-eye" aria-hidden="true"></i>
									</a> 
									@endif
									@if(in_array("tvideo_all",$rolerawdata, TRUE) || in_array("tvideo_write",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))             
									<a href="{{route('edit.tvideos', $tvideos->id)}}" class="btn btn-sm btn-info align-self-center" style="border-radius: 104px;padding: 8px 8px 10px 13px;margin: 0px 1px;">
										<i class="fa fa-edit" aria-hidden="true"></i>
									</a>
									@endif
									@if(in_array("tvideo_all",$rolerawdata, TRUE) || in_array("tvideo_delete",$rolerawdata, TRUE)||in_array("kt_roles_select_all",$rolerawdata, TRUE))
									<a href="#" onclick="deleteConfirmation({{$tvideos->id}})" data-id="{{ $tvideos->id }}" class="btn btn-sm crop-delete btn-danger align-self-center" style="border-radius: 100px;padding: 8px 8px 8px 12px;">
	...
	function deleteConfirmation(id)
 	{
		swal({
			...
			let _url = `/corporate/video/destroy/${id}`;
			$.ajax({
				type: 'POST',  
				url: _url,
				data: {_token: token},  
